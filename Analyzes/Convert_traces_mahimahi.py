#!/usr/bin/env python3
"""
convert_all_traces.py
=====================
Converts trimmed ICMP ping logs and UDP pcap files to mahimahi format
for all 9 static traces.
Calls parse_recorder_to_replayer.py for each trace and direction.
Input (from trimmed_traces_20260207):
    icmp_ul_trimmed.log   uplink ICMP ping log
    udp_ul_trimmed.pcap   uplink UDP pcap
Output saved to new_mahimahi_traces_seq_based/<trace_id>/uplink/:
    delay_uplink.txt   one-way delay per sequence number
...
OUTPUT_BASE  = "/mnt/nuwinslab_nas/nuwins_data_platform/datasets/bos_phi_trip_202601/new_mahimahi_traces_seq_based"
SCRIPT       = os.path.expanduser("~/graphs/parse_recorder_to_replayer.py")
TRACES = [
    "1770506564594-0500",
    "1770506824350-0500",
    "1770507107930-0500",
...
if __name__ == "__main__":
    for trace_id in TRACES:
        ...
        if not os.path.exists(pcap_file):
            ...
            continue
        # Run parse_recorder_to_replayer.py
        cmd = [
            "python3", SCRIPT,
            "--ping_file",  ping_file,
            "--pcap_file",  pcap_file,
        ...
        if result.returncode == 0:
            # Rename output files to standard mahimahi names
            os.rename(
